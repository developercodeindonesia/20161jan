...
switch($_GET['act']){
  // Tampil Produk
  default:
    echo "<h2 class='hLine'>Artikel</h2>
          <input type=button class='butt' value='Tambah Artikel Baru' onclick=\"window.location.href='?module=artikel&act=addArtikel';\">
          <table class='list'>
          <thead>
			  <tr>
    ...
    $tampil = mysql_query("SELECT * FROM artikel ORDER BY id_artikel DESC LIMIT {$startpoint}, {$limit}");
    ...
    while($r=mysql_fetch_array($tampil)){
      ...
      echo "<tr><td>$no</td>
                ...
                <td>$r[tanggal]</td>
		            <td><a href=?module=artikel&act=editArtikel&id=$r[id_artikel]><img src='images/add-icon.gif'></a></td> 
		            <td><a href=$aksi?module=artikel&act=del&id=$r[id_artikel]&nama_file=$r[gambar]><img src='images/hr.gif'></a></td>
		        </tr>";
      $no++;
    }
    echo "</table>";
  ...
  case "addArtikel":
    echo "<h2 class='hLine'>Tambah Artikel</h2>
      ...
      <td class='left'><input type=text name='oleh' size=60></td>
      </tr>
		  <tr>
  ...
  case "editArtikel":
    ...
    echo "<h2 class='hLine'>Edit Detail Artikel</h2>
      ...
      <td class='left'><input type=text name='oleh' size=60 value='$r[oleh]'></td>
		   ...
		   <tr>
			<td  class='left'>Gambar</td>
			<td  class='left'>  
			<img src='../joimg/artikel/s_$r[gambar]'></td>
		  </tr>
          <tr>
